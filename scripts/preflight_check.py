#!/usr/bin/env python3
"""Validate Seen-It-First deployment prerequisites."""
...
def _load_yaml(path: Path, label: str, ctx: CheckContext) -> dict:
    ...
    if not isinstance(parsed, dict):
        ctx.error(
            f"Invalid {label} root type at {path}: expected dict, found {type(parsed).__name__}"
        )
        return {}
...
def check_writable_paths(repo_root: Path, ctx: CheckContext) -> None:
    for rel in ("data", "logs"):
        ...
        try:
            with probe.open("w", encoding="utf-8") as file_obj:
                file_obj.write("ok\n")
            probe.unlink()
...
def _validate_camera(cam_id: str, cam_def: dict, ctx: CheckContext) -> None:
    ...
    for field in required_int_fields:
        ...
        try:
            ...
            ctx.error(
                f"Camera {cam_id} has non-integer value for cameras.{cam_id}.{field}: {cam_def[field]!r}"
            )
            continue
        if field != "sensor_id" and value <= 0:
            ctx.error(
                f"Camera {cam_id} has invalid non-positive value for cameras.{cam_id}.{field}: {value}"
            )
    ...
    if cam_type == "rtsp":
        ...
        if not isinstance(uri, str) or not uri.strip():
            ctx.error(
                f"Camera {cam_id} requires non-empty URI for cameras.{cam_id}.uri when type=rtsp"
            )
...
def check_camera_config(camera_cfg: dict, ctx: CheckContext) -> None:
    ...
    if not enabled:
        ctx.error(
            "No enabled cameras found in edge/camera/config.yaml (expected at least one enabled=true)"
        )
        return
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate Seen-It-First deployment prerequisites."
    )
    parser.add_argument(
        "--system-config",
